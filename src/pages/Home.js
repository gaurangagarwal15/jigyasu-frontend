...
const Home = ({ token, setToken }) => {
  ...
  const fetchListings = async () => {
    ...
    try {
      const response = await fetch(
        "https://jigyasu.up.railway.app/api/event/all"
      );
  ...
  return (
    <>
      ...
      {listings.length >= 1 ? (
        <>
          <h2 className="text-2xl py-3 px-6 text-blue-600 mt-2 ml-3 font-semibold">
            Recently Added
          </h2>
        ...
        <div className="text-xl px-5 py-5 mt-30 text-center">
          :( Nothing to show!
        </div>
      )}
    </>
