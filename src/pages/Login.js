...
const Login = ({ token, setToken }) => {
  ...
  const LoginStatusMessage = (props) => {
    if (status === 401) {
      return (
        <div className="bg-blue-200 text-sm text-blue-500 rounded px-1 py-1">
          {errorMessage}
          <br />
    ...
    if (errorMessage) {
      return (
        <div className="bg-red-300 text-xs text-white rounded px-1 py-1">
          {errorMessage}
        </div>
      );
    } else {
  ...
  const handleLogin = (event) => {
    ...
    setStatus("");
    if (email && password) {
      fetch("https://jigyasu.up.railway.app/api/user/login", {
        method: "POST",
        headers: {
  ...
  if (params.method == "verify") {
    fetch(
      `https://jigyasu.up.railway.app/api/user/verify?code=${params.code}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
      ...
      <div className="flex justify-center mt-10">
        <div class="w-full max-w-lg">
          ...
          >
            ...
            <button
              className={`w-full ${
                loading ? "bg-gray-300" : "bg-gray-900 hover:bg-blue-700"
              }  text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline`}
