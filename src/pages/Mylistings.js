...
const MyListings = ({ token, setToken }) => {
  ...
  const fetchMyData = async () => {
    ...
    try {
      const response = await fetch(
        "https://jigyasu.up.railway.app/api/user/me",
        {
          headers: {
  ...
  const fetchUserEvents = async () => {
    if (userData._id) {
      ...
      try {
        const response = await fetch(
          `https://jigyasu.up.railway.app/api/user/find/${userData?._id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(
        "https://jigyasu.up.railway.app/api/listings/delete",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      ...
      {userData && listings && (
        <div className="p-2 max-w-6xl margin-auto rounded  m-auto mt-10 p-2">
          <div className="flex gap-2 p-3 bg-blue-900 rounded-xl text-white">
            ...
            <div>
              ...
              <p className="font-semibold text-sm ml-10">
                {" "}
            ...
            <div className="ml-36">
              <p1 className="text-xl font-semibold">🏆 Stats 🏆</p1>
              <br />
              Participations : {userData?.event_partcipated?.length}
              <br />
              Hosted : {userData?.events_hosted?.length}
            </div>
            <div className="ml-36">
              <p1 className="text-xl font-semibold">✨ Intrests ✨</p1>
              <br />
            </div>
          </div>

          <h1 className="text-2xl text-blue-900 font-semibold -mb-16 mt-10 ml-10">
            Events Hosted
          </h1>
                  ...
                  <>
                    {event?.event_participated && (
                      <Product
                        ...
                        description={
                          event?.event_participated?.eventDescription
                        }
