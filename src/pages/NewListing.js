...
const NewListing = ({ token, setToken }) => {
  ...
  const handleProductAdd = (event) => {
    ...
    if (pName && productImage && category && description && expPrice && age) {
      fetch("https://jigyasu.up.railway.app/api/listing/new", {
        method: "POST",
        headers: {
  ...
  const handleProductImageChange = async (event) => {
    ...
    const resp = await fetch(
      "https://ecomm-backend-xcv34.herokuapp.com/api/listing/newImg",
      {
        method: "POST",
        headers: {
      ...
      <div className="flex justify-center mt-10">
        <div class="w-full max-w-xl">
          ...
          >
            <div className="mb-4">
              {error && (
                <div className="bg-red-300 text-base text-white rounded px-1 py-1">
                  {status}
                </div>
              )}
              {success && (
                <div className="bg-green-500 text-base text-white rounded px-1 py-1">
                  {status}
                </div>
              )}
            </div>
