...
const ProductPage = ({ token, setToken }) => {
  ...
  const fetchListings = async () => {
    setLoading(true);
    var endpoint = `https://jigyasu.up.railway.app/api/event/findbyCode/${params.id}`;
    try {
  ...
  const fetchPartcipants = async () => {
    setLoading(true);
    var endpoint = `https://jigyasu.up.railway.app/api/event/find/${params.id}`;
    try {
      const response = await fetch(endpoint);
  ...
  const handleSendNotification = (event) => {
    ...
    setSubLoading(true);
    fetch(
      `https://jigyasu.up.railway.app/api/event/email/${listings?.event_random_code}?subject=${subject}&context=${message}`,
      {
        method: "GET",
        headers: {
  ...
  const handleSendNotificationtoAll = (event) => {
    ...
    setSubLoading(true);
    fetch(
      `https://jigyasu.up.railway.app/api/user/email/?subject=${subject}&context=${message}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-gray-500 xl:mx-12">
              ...
              it for your personal or commercial events. Just don't forget to
              ...
              <div className="pricing-plan border-t-4 border-solid border-white bg-white text-center max-w-sm mx-auto hover:border-indigo-600 transition-colors duration-300">
                ...
                <div className="p-6">
                  ...
                  <div className="mt-6 py-4">
                    <button
                      onClick={() => setOption(4)}
                      className="bg-indigo-600 text-xl text-white py-2 px-6 rounded hover:bg-indigo-700 transition-colors duration-300"
                    >
                      Pay Now
                    </button>
                  </div>
      ...
      <div className="flex flex-col justify-center px-10 max-w-4xl mx-auto mb-16">
        {listings && particpants && (
          <>
            ...
            <h2 className="mt-5 md:mt-1 text-2xl">{listings.eventName}</h2>
            <br />
            <p className="font-semibold text-xl">
              Total Registrations :{" "}
              <span className="text-pink-600">{particpants.length}</span>
            </p>
            <div className="mt-6 p-2">
              <h3 className="text-xl font-semibold mb-5">Actions</h3>
              <div className="flex justify-around">
                <button
                  className="bg-blue-800 rounded p-2 text-white font-semibold"
                  onClick={() => setOption(1)}
                >
                  🔔 Send Notification Email
                </button>
                <button
                  className="bg-gray-900 rounded p-2 text-white font-semibold"
                  onClick={() => setOption(0)}
                >
                  👁️ View Particants
                </button>
                <button
                  className="bg-red-800 rounded p-2 text-white font-semibold"
                  onClick={() => setOption(2)}
                >
                  🏅Release Certificates
                </button>
                <button
                  className="bg-cyan-800 rounded p-2 text-white font-semibold"
                  onClick={() => setOption(3)}
                >
                  🚀Promote
                </button>
                            ...
                            <div class="flex items-center space-x-4">
                              ...
                              <div class="flex-1 min-w-0">
                                ...
                                <p class="text-sm text-gray-900 truncate">
                                  <a href={`mailto:${particpant.email}`}>
                                    {particpant.email}
                                  </a>
                                </p>
                                <p class="text-sm text-gray-900 truncate ">
                  ...
                  <form onSubmit={handleSendNotification}>
                    <div class="mb-6 mt-10">
                      ...
                      <input
                        ...
                        id="message"
                        rows="4"
                        class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-blue-800 focus:ring-blue-500 focus:border-blue-500  "
                    ...
                    <button
                      type="submit"
                      class="text-white bg-blue-700 hover:bg-blue-800 w-full focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2"
                    >
                      {subLoading ? "sending..." : "Send message"}
                    </button>
                  </form>
                </>
              )}
